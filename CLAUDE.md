...
8. **Consumer-facing public pages** — Generated apps should support a public-facing mode (no auth required) for end-customer interactions. Example: a scheduling app where the business's customers can visit a link to book an appointment, while the business manages/views bookings from their authenticated app. This enables B2B2C use cases — our business customers embed or share a link on their website that their end-customers can use. Needs: public route support in generated apps, separate auth-free pages, data flows back to the business's authenticated dashboard.
