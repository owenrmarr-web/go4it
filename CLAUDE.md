...
18. **GoChat iOS app (Capacitor)** — Wrap GoChat (Next.js app at `apps/gochat/`) in a Capacitor shell for native iOS distribution via App Store. Push notifications required (not possible with PWA-only on iOS). Setup: `@capacitor/core`, `@capacitor/ios`, `@capacitor/push-notifications`. Needs Apple Developer account ($99/yr), APNs configuration, and App Store review. GoChat's existing Next.js UI runs in a WKWebView with native bridges for push notifications, haptics, etc.
