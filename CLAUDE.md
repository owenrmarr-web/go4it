...
1. **Fix Create page UI to show preview URL (BLOCKING)** — The Create page `/create` shows "done" prematurely (stale localStorage state from previous generation) and doesn't display the preview URL once the Fly preview is actually ready. The UI should: (a) not show "done" until the Fly preview app is deployed and healthy, (b) show the preview `.fly.dev` URL with a clickable link once ready, (c) clear stale terminal state from previous generations on page load. Related to the old "app is ready" flash bug. Key files: `src/components/GenerationContext.tsx`, `src/components/GenerationProgress.tsx`, `src/app/create/page.tsx`.
2. **Instant launch via secret flip (IN PROGRESS)** — "Go Live" on Account page should take ~15-25s (secret flip on existing preview Fly app) instead of ~3-5 min (full Docker rebuild). Unified preview/production Dockerfile implemented (`builder/src/lib/fly.ts`): preview image includes full `node_modules/`, `prisma/`, and smart `start.sh` that branches on `PREVIEW_MODE` env var. `launchApp()` function added to builder. **Remaining:** verify the `isPreviewLaunch` flag reaches the builder correctly (debug logging added to both platform and builder deploy routes), test the instant launch end-to-end. Key files: `builder/src/lib/fly.ts` (`launchApp()`), `builder/src/routes/deploy.ts`, `src/app/api/organizations/[slug]/apps/[appId]/deploy/route.ts`.
...
9. **Consumer-facing public pages** — Generated apps should support a public-facing mode (no auth required) for end-customer interactions. Example: a scheduling app where the business's customers can visit a link to book an appointment, while the business manages/views bookings from their authenticated app. This enables B2B2C use cases — our business customers embed or share a link on their website that their end-customers can use. Needs: public route support in generated apps, separate auth-free pages, data flows back to the business's authenticated dashboard.
