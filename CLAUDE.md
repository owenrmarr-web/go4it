...
6. **GoSchedule — customer-facing version** — Build a polished, customer-facing scheduling app (GoSchedule) as the flagship B2B2C demo. Business owner manages services, availability, and team calendar from an authenticated dashboard. Their customers visit a public booking page (no auth required) to browse available slots, book appointments, and pay. This proves the B2B2C model and becomes the template for how all generated apps can support public-facing pages. Ties into payments (#7) and consumer-facing pages (#8).
...
15. **Multi-org visibility in account dashboard** — Account page currently shows one org. Users who are members of multiple orgs (or own one org and are members of another) should see all their orgs with a switcher or stacked sections. Schema already supports multiple `OrganizationMember` rows per user.
16. **Multi-org ownership** — Allow a single user to own multiple organizations. Currently 1:1 org simplification auto-creates one org on owner signup. Needs: org creation flow from account page, org switcher in header, scoped API routes.
17. **Member avatar sync to deployed apps (Phase 2)** — Sync `profileColor`, `profileEmoji`, and `image` from platform User model to deployed apps via `GO4IT_TEAM_MEMBERS` secret or team-sync endpoint. Playbook update to render member avatars (photo/emoji/initials on color background) in generated app UI.
18. **POS integration** — Connect generated apps to point-of-sale systems so businesses can accept payments, sync inventory, and pull real sales data. Priority targets by SMB adoption: (1) **Square** — largest SMB install base, free REST API, no approval needed, OAuth for third-party apps. Key endpoints: Orders, Payments, Inventory, Customers, Catalog. (2) **Shopify POS** — popular with retail businesses that also sell online. GraphQL Admin API + REST. (3) **Clover** — common in restaurants/retail (Fiserv-owned), REST API. (4) **Toast** — restaurant-focused (200K+ locations), requires partner approval. Start with Square: add a "Connect Square" OAuth pattern to the playbook so generated apps can accept payments on public-facing pages (ties into #8 B2B2C) and pull sales data for dashboards/analytics. Playbook needs: OAuth flow template, payment collection on public routes, sales data sync pattern.
