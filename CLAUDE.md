...
1. ~~Fix Create page UI to show preview URL (FIXED)~~ — See "Completed" section below.
2. ~~Instant launch via secret flip (FIXED)~~ — See "Completed" section below.
3. **Email verification (IN PROGRESS)** — On signup, redirect to "check your email" page. Resend sends a verification link that, when clicked, marks the user verified and opens the home page. Fast email delivery is critical. Also require email verification for password changes later.
4. **Billing** — Track per-user Fly.io usage, charge 20% premium. Stripe integration.
5. **Builder hardening** — Garbage collection for old workspaces, rate limiting, error logging/monitoring.
6. **Consumer-facing public pages** — Generated apps should support a public-facing mode (no auth required) for end-customer interactions. Example: a scheduling app where the business's customers can visit a link to book an appointment, while the business manages/views bookings from their authenticated app. This enables B2B2C use cases — our business customers embed or share a link on their website that their end-customers can use. Needs: public route support in generated apps, separate auth-free pages, data flows back to the business's authenticated dashboard.
7. **Model selection toggle** — Let users choose Sonnet (fast/cheap) vs Opus (slower/higher quality) on the Create page. Plan written but deferred. See `/Users/owenmarr/.claude/plans/reflective-tumbling-gizmo.md`.
...
10. **Fix custom subdomain DNS** — `*.go4it.live` CNAME points to `fly-global.fly.dev` which returns NXDOMAIN. Options: (a) per-app CNAME records via DNS API, or (b) shared reverse-proxy Fly app for wildcard routing. Currently using `.fly.dev` URLs as workaround.
11. **Custom domains (phase 2)** — Support user-owned domains like `crm.mybusiness.com` (CNAME validation + Fly.io per-app certs).
12. **App store preview mode** — Better preview experience for marketplace apps before deploying. Details TBD.
