...
1. **Fix auto-deploy build failures (BLOCKING)** — Generated apps consistently fail `npm run build` during the finalize stage, preventing auto-deploy to Fly.io. The CLI generates the app successfully (exit code 0), but the post-generation build validation fails. Need to: (a) SSH into builder and run `npm run build` manually on a generated workspace to capture the actual error output, (b) determine if it's a code issue (playbook needs guardrails) or environment issue, (c) fix root cause. The `tryBuild` error detection has been improved (added `⨯` to filter, `.next/standalone` existence check, stale `.next/lock` cleanup) but the underlying build failure persists across 4+ test generations.
2. **Fix "app is ready" UI flash** — When navigating to `/create` via client-side navigation, stale `stage: "complete"` from a previous generation flashes briefly. Current `useEffect`-based reset runs after first render. Fix: check `didClearStaleRef` in `derivePageState()` synchronously to suppress stale terminal state before the effect fires. Plan written in `/Users/owenmarr/.claude/plans/happy-munching-minsky.md`.
