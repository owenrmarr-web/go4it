...
import type { CurrentUser } from "@/components/ChatLayout";
...
export default function ProfileModal({
  ...
  // Fetch current OOO status on mount
  useEffect(() => {
    fetch("/api/presence")
      .then((r) => r.json())
      .then((users) => {
        const me = users.find((u: { id: string }) => u.id === currentUser.id);
        if (me) {
          setIsOOO(me.isOOO || false);
          setOooMessage(me.oooMessage || "");
          setOooUntil(me.oooUntil ? new Date(me.oooUntil).toISOString().split("T")[0] : "");
        }
      })
      .catch(() => {});
  }, [currentUser.id]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedFile) {
        ...
        // Clear avatar image, update color
          ...
          {isPlatformManaged ? (
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 text-center">
                ...
                {/* Link omitted: users edit profile on the GO4IT platform account page */}
              </p>
            </div>
          ) : (
            <>
        ...
        <div className="px-4 pb-4">
          <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Out of Office</p>
                <p className="text-xs text-gray-400">Auto-reply when someone DMs you</p>
              </div>
              <button
                onClick={() => setIsOOO(!isOOO)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  isOOO ? "bg-purple-600" : "bg-gray-300 dark:bg-gray-600"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    isOOO ? "translate-x-6" : "translate-x-1"
                  }`}
                />
              </button>
            </div>
            {isOOO && (
              <div className="space-y-2 mt-3">
                <input
                  ...
                  className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                  maxLength={200}
                />
                <div>
                  <label className="text-xs text-gray-400 mb-1 block">Return date (optional)</label>
                  <input
                    type="date"
                    value={oooUntil}
                    ...
                    try {
                      await fetch("/api/presence", {
                        ...
                        body: JSON.stringify({
                          ...
                          oooUntil: oooUntil || null,
                        }),
                      });
                      toast.success("Out of Office updated");
                    } catch {
                      toast.error("Failed to update OOO status");
                    } finally {
                      setSavingOOO(false);
                    }
                  }}
                  disabled={savingOOO}
                  className="w-full px-3 py-1.5 text-xs font-medium text-white gradient-brand rounded-lg hover:opacity-90 disabled:opacity-50"
                >
                  {savingOOO ? "Saving..." : "Save OOO Status"}
                </button>
              </div>
            )}
            {!isOOO && (
                ...
                onClick={async () => {
                  ...
                  try {
                    await fetch("/api/presence", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ isOOO: false, oooMessage: null, oooUntil: null }),
                    });
